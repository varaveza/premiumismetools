  ...
  try {
    ...
    if (regResponse.message === "success") {
      ...
      if (verifyResponse.message === "success") {
        ...
        return { success: true, ip: currentIP, country: currentCountry, domain: currentDomain, email: currentEmail, password };
      } else {
  ...
  while (successCount < totalAccounts && attempts < maxAttempts) {
    ...
    const batchTasks = Array.from({ length: batchSize }, (_, i) => {
      ...
      return limit(async () => {
        ...
        if (result.success) {
          successCount++;
          successes.push({ email: result.email, password: password || config.password, country: result.country });
        } else {
          failCount++;
        }
...
// --- Rate limit storage (file-based), similar approach to spotify-creator API ---
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const USAGE_DIR = path.join(__dirname, 'logs');
const USAGE_FILE = path.join(USAGE_DIR, 'usage.json');
const MAX_GLOBAL_PER_DAY = process.env.MAX_GLOBAL_PER_DAY ? parseInt(process.env.MAX_GLOBAL_PER_DAY, 10) : 300; // sesuai requirement
const MAX_PER_IP_PER_DAY = process.env.MAX_PER_IP_PER_DAY ? parseInt(process.env.MAX_PER_IP_PER_DAY, 10) : 25;  // sesuai requirement
...
function isLocalIp(ip) {
  ...
  // Also allow common docker/localhost representations if needed in future
  return false;
}
